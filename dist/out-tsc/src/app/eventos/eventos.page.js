var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { Component } from "@angular/core";
import { TimelineService } from "../services/timeline-service";
import { PacientedtoService } from "../services/pacientedto-service";
import { Memorando } from "../models/memorando";
import { ModalController } from "@ionic/angular";
import { MemoPage } from "../memo/memo.page";
import { Router } from "@angular/router";
import { PhotoModalPage } from "../photo-modal/photo-modal.page";
import { PacientePhotoService } from "../services/pacientephoto-service";
import { PacientePhoto } from "../models/paciente-photo";
import { AuthServerProvider } from "../services/auth-jwt.service";
import { PhotoDTO } from "../photo-modal/photo-dto";
import { Evento } from "../models/evento";
import { EventoModalPage } from "../evento-modal/evento-modal.page";
import { EventoRotina } from "../models/eventorotina";
var EventosPage = /** @class */ (function () {
    function EventosPage(timelineService, pacientedtoService, modalController, router, pacientePhotoService, authServerProvider) {
        this.timelineService = timelineService;
        this.pacientedtoService = pacientedtoService;
        this.modalController = modalController;
        this.router = router;
        this.pacientePhotoService = pacientePhotoService;
        this.authServerProvider = authServerProvider;
        this.timelines = new Array();
        this.eventoRotina = new EventoRotina();
    }
    EventosPage.prototype.ngOnInit = function () {
        console.log("TAB EVENTO");
        this.pacientedto = this.pacientedtoService.pacienteDTO;
        this.loadTimeline();
    };
    EventosPage.prototype.addEvento = function () {
        return __awaiter(this, void 0, void 0, function () {
            var evento, modalEvento;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        evento = new Evento();
                        evento.paciente = this.pacientedto.paciente;
                        return [4 /*yield*/, this.modalController.create({
                                component: EventoModalPage,
                                componentProps: {
                                    evento: evento
                                }
                            })];
                    case 1:
                        modalEvento = _a.sent();
                        return [4 /*yield*/, modalEvento.present()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, modalEvento.onDidDismiss().then(function (data) {
                                if (data.data) {
                                    _this.loadTimeline();
                                }
                            })];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    EventosPage.prototype.editable = function (usuario) {
        var editable = false;
        this.authServerProvider.user().then(function (login) {
            if (login === usuario.login) {
                editable = true;
            }
        });
        return editable;
    };
    EventosPage.prototype.editEvento = function (evento) {
        return __awaiter(this, void 0, void 0, function () {
            var editable, modalEvento;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        editable = this.editable(evento.usuario);
                        return [4 /*yield*/, this.modalController.create({
                                component: EventoModalPage,
                                componentProps: {
                                    evento: evento,
                                    editable: editable
                                }
                            })];
                    case 1:
                        modalEvento = _a.sent();
                        return [4 /*yield*/, modalEvento.present()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, modalEvento.onDidDismiss().then(function (data) {
                                if (data.data) {
                                    _this.loadTimeline();
                                }
                            })];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    EventosPage.prototype.loadTimeline = function () {
        var _this = this;
        this.timelineService.get(this.pacientedto.paciente.idpaciente).subscribe(function (res) {
            _this.timelines = res.body;
        }, function (err) {
            console.log(err.error);
        });
    };
    EventosPage.prototype.addMemorando = function () {
        return __awaiter(this, void 0, void 0, function () {
            var memorando, modalMemorando;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        memorando = new Memorando();
                        memorando.paciente = this.pacientedto.paciente;
                        return [4 /*yield*/, this.modalController.create({
                                component: MemoPage,
                                componentProps: {
                                    memorando: memorando
                                }
                            })];
                    case 1:
                        modalMemorando = _a.sent();
                        return [4 /*yield*/, modalMemorando.present()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, modalMemorando.onDidDismiss().then(function (data) {
                                if (data.data) {
                                    _this.loadTimeline();
                                }
                            })];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    EventosPage.prototype.editMemorando = function (memorando) {
        return __awaiter(this, void 0, void 0, function () {
            var modalMemorando;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        memorando.paciente = this.pacientedto.paciente;
                        return [4 /*yield*/, this.modalController.create({
                                component: MemoPage,
                                componentProps: {
                                    memorando: memorando,
                                    editable: this.editable(memorando.usuario)
                                }
                            })];
                    case 1:
                        modalMemorando = _a.sent();
                        return [4 /*yield*/, modalMemorando.present()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, modalMemorando.onDidDismiss().then(function (data) {
                                if (data.data) {
                                    _this.loadTimeline();
                                }
                            })];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    EventosPage.prototype.addPhoto = function () {
        return __awaiter(this, void 0, void 0, function () {
            var photodto, modalPhoto;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        photodto = new PhotoDTO();
                        return [4 /*yield*/, this.modalController.create({
                                component: PhotoModalPage,
                                componentProps: {
                                    photodto: photodto
                                }
                            })];
                    case 1:
                        modalPhoto = _a.sent();
                        return [4 /*yield*/, modalPhoto.present()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, modalPhoto.onDidDismiss().then(function (data) {
                                if (data.data) {
                                    var photodto_1 = data.data;
                                    var pacientePhoto = new PacientePhoto();
                                    pacientePhoto.descricao = photodto_1.descricao;
                                    pacientePhoto.photo = photodto_1.photo;
                                    pacientePhoto.dataregistro = new Date();
                                    pacientePhoto.paciente = _this.pacientedto.paciente;
                                    /*   this.authServerProvider.usuario().then(usuario => {
                                         pacientePhoto.usuario = usuario;
                                       });*/
                                    /*this.authServerProvider.getUsuario().then(usuario =>{
                                    pacientePhoto.usuario = usuario;
                                   })*/
                                    pacientePhoto.principal = false;
                                    _this.pacientePhotoService.save(pacientePhoto).subscribe(function (res) {
                                        _this.loadTimeline();
                                    }, function (err) {
                                        console.log(err.error);
                                    });
                                }
                            })];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    EventosPage.prototype.editPhoto = function (pacientePhoto) {
        return __awaiter(this, void 0, void 0, function () {
            var photodto, modalPhoto;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        photodto = new PhotoDTO();
                        photodto.descricao = pacientePhoto.descricao;
                        photodto.photo = pacientePhoto.photo;
                        return [4 /*yield*/, this.modalController.create({
                                component: PhotoModalPage,
                                componentProps: {
                                    photodto: photodto,
                                    editable: this.editable(pacientePhoto.usuario)
                                }
                            })];
                    case 1:
                        modalPhoto = _a.sent();
                        return [4 /*yield*/, modalPhoto.present()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, modalPhoto.onDidDismiss().then(function (data) {
                                if (data.data) {
                                    photodto = data.data;
                                    pacientePhoto.descricao = photodto.descricao;
                                    pacientePhoto.photo = photodto.photo;
                                    _this.pacientePhotoService.update(pacientePhoto).subscribe(function (res) {
                                        _this.loadTimeline();
                                    });
                                }
                            })];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    EventosPage.prototype.voltar = function () {
        this.router.navigate(["home"]);
    };
    EventosPage = __decorate([
        Component({
            selector: "app-eventos",
            templateUrl: "./eventos.page.html",
            styleUrls: ["./eventos.page.scss"]
        }),
        __metadata("design:paramtypes", [TimelineService,
            PacientedtoService,
            ModalController,
            Router,
            PacientePhotoService,
            AuthServerProvider])
    ], EventosPage);
    return EventosPage;
}());
export { EventosPage };
//# sourceMappingURL=eventos.page.js.map